...
import { selectSourceAssetOption, useSourceAssetOptions } from "../data/sourceAssets"
 ...
 * asset and chain selectors cover every supported source route while the
 * address remains keyed only by the destination. Auto-advances to tracking
 * once a deposit is detected. Source metadata resolves from the Router (Skip),
 * whose identifiers match the Deposit API's src_chain_id / src_denom.
 */
const DepositAddress = () => {
  ...
  const selectedAsset = selectSourceAssetOption(options, selectedAssetSymbol)
  const activeAssetSymbol = selectedAsset?.symbol ?? ""
  ...
  const assetOptions: SourceOption[] = options.map((option) => ({
    value: option.symbol,
    label: option.symbol,
    logoUrl: option.logoUrl,
  }))

  // The fresh variant withholds cached address data until this mount reissues
  // the detection cursor. A failed reissue surfaces through the error branch
  // below with no Retry button — backing out and re-entering already retries.
  const {
    query,
  ...
  const activeDepositId = activeDeposits.data?.[0]?.id ?? ""
  ...
  return (
    ...
    >
      <div className={styles.root}>
        ...
        {!hardError && activeDepositId && (
          <button
            type="button"
            className={styles.resume}
            onClick={() => trackDeposit(activeDepositId)}
          >
