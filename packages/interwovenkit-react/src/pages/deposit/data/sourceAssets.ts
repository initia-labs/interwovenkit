...
/**
 * Source-asset options for a destination, grouped by display symbol. First-seen
 * order is kept.
 */
...
/** Keeps a valid selection across metadata updates and falls back to the first option. */
export function selectSourceAssetOption(
  options: SourceAssetOption[],
  selectedSymbol: string,
): SourceAssetOption | undefined {
  return options.find((option) => option.symbol === selectedSymbol) ?? options[0]
}
