...
/**
 * Preserves the detected record before navigating so tracking can poll that
 * exact deposit instead of selecting among every transfer at a reused address.
 */
export function useTrackDeposit() {
  ...
  return useCallback(
    (depositId: string) => {
      setValue("trackedDepositId", depositId)
      navigate("track")
    },
    [setValue, navigate],
  )
