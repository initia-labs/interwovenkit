...
// Unsent only if neither the mined nor the pending nonce moved past the values read before the prompt.
export function checkHashlessSend(
  ...
  const unchanged =
    ...
    read?.latest === promptNonce &&
    read.pending === promptPendingNonce
  return {
