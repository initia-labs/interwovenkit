...
interface BridgeTxOptions {
  fee?: StdFee
}

export function useBridgeTx(tx: TxJson, options: BridgeTxOptions = {}) {
  const { fee } = options
...
export function useTxStatusQuery(details: HistoryDetails) {
  const { timestamp, chainId, txHash, tracked, state } = details
  const skip = useSkip()
