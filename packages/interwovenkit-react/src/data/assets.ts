...
function useCreateAssetListQuery() {
...
export function useAssets(chain?: NormalizedChain) {
  const createAssetListQuery = useCreateAssetListQuery()
  const { data } = useSuspenseQuery(createAssetListQuery(chain))
  return data
}

export function useAllChainAssetQueries() {
  const chains = useInitiaRegistry()
  const createAssetListQuery = useCreateAssetListQuery()
  return useQueries({
    queries: chains.map((chain) => createAssetListQuery(chain)),
  })
}
