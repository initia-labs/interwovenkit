...
import { useAllChainAssetQueries, type NormalizedAsset } from "./assets"
import { useAllChainBalanceQueries } from "./account"
...
export function sortChainsWithinGroup(chains: AssetBalance[]): AssetBalance[] {
  return sortWith<AssetBalance>([descend(prop("value")), ascend(({ chain }) => chain.name)])(chains)
}
...
function createAssetGroup(assetBalance: AssetBalance): AssetGroup {
  return {
    ...
    chains: [assetBalance],
    totalQuantity: fromBaseUnit(assetBalance.balance, { decimals: assetBalance.asset.decimals }),
    totalValue: assetBalance.value,
  }
}
...
function addToAssetGroup(group: AssetGroup, assetBalance: AssetBalance): AssetGroup {
  const newQuantity = fromBaseUnit(assetBalance.balance, { decimals: assetBalance.asset.decimals })
  return {
    ...group,
    chains: [...group.chains, assetBalance],
    totalQuantity: BigNumber(group.totalQuantity).plus(newQuantity).toString(),
    totalValue: group.totalValue + assetBalance.value,
  }
}
...
function createAssetBalance(
  ...
  const quantity = fromBaseUnit(balance.amount, { decimals: asset.decimals })
  return {
    chain,
    asset,
...
export function aggregatePortfolio(
  ...
  const { supported, unsupported } = chains.reduce<{
    ...
    (acc, chain, chainIndex) => {
      ...
      if (!balances || !assets || !prices) return acc
...
export const usePortfolio = () => {
  ...
  const queries = {
    balances: useAllChainBalanceQueries(),
    assets: useAllChainAssetQueries(),
