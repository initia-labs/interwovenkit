...
- **BigNumber strict mode**: Use `|| 0` (or `|| "0"`) as the _final_ fallback before passing a value to `BigNumber()` or any method that accepts a BigNumber-coercible argument (`.plus()`, `.times()`, `.minus()`, `.div()`, `.gt()`, `.gte()`, `.lt()`, `.lte()`, `.eq()`, `.comparedTo()`, etc., since they internally construct BigNumbers from their arguments). `?? "0"` is insufficient — nullish coalescing only catches `null`/`undefined`, not empty strings. This matters because the peer dep allows `bignumber.js@>=9` and v10 removed the opt-in `DEBUG` flag, making throws on invalid input the default; v11 keeps this as the default behind a new `STRICT` option (defaults to `true`, opt-out only). The realistic empty-string source is `@initia/utils`' `fromBaseUnit`/`toBaseUnit`, which return `""` on invalid input; Coin/route amount fields typed `string` can also arrive empty from upstream. The vitest setup keeps `BigNumber.DEBUG = true` so every test acts as a strict-mode regression check.
